...
function displayPosts() {
    if (localStorage.getItem("posts") !== null){
        ...
        for (var i = 0; i < data.length; i++) {
			...
			status ="<td class='status'>" + '<a href="index.1.7_mitsuggestionbox_inProgress.php"><div class="status sc'+data[i].status +'" >'+ '</div></a>' + "</td>";
			upvoter='<td class="votes"><span class="vote"> </span><span class="votes">2</span></td>';
            
			table += '<tr>'+status + upvoter+'<td>' + nl2br(processIdea(data[i].body)) + "</td>" + 
...
function submitPostAndGetPosts() {
    $.ajax({
            ...
            'data': {'newpost': $('#newpost').val(),'ideatitle': extractIdeaName($('#newpost').val())},
