	...
	function displayPosts() {
		if (localStorage.getItem("posts") !== null){
			...
			var progbarlist="<ul class='progbarlist'>";
				for (var i = 0; i < data.length; i++) {	
					rowObjToHTMLRow(data[i])
					
			function rowObjToHTMLRow(rowObj) {
				
					progbarlist+="<li>";
					
						var table = "<table class='table'>" // <tr> <th>Post Body</th>  <th></th>Progress Bar<th>User</th> <th>Time</th> </tr>";    
			
							var time = new Date(rowObj.time * 1000);
							//nl2br(processIdea(rowObj.body))
							n=extractIdeaName(rowObj.body)		
			
					var statusTable={0:"Not acknowledged",1:"Acknowledged",2:"In Progress", 3:"Done"};
					var progEntry=rowObj.progress && rowObj.progress != "null" ? rowObj.progress + '% - ': "";
					
					status ="<td class='status'>" + '<a href="index.1.7_mitsuggestionbox_inProgress.php" rel="popover" data-content="'+statusTable[rowObj.status]+'" data-original-title=""><div class="status sc'+rowObj.status +'" >'+ '</div></a>' + "</td>";
							
									
								var tablerow = "<tr>"+status+
									"<td style='width:150px'>" + '<a href="#" class="idea" rel="popover" data-content="'+rowObj.body+'" data-original-title="">'+n + '</a>' + "</td>" + 
									"<td class='progbarcell' style='width:400px;padding-right:100px'>" + 
										'<!-- **PROGBAR** -->      <div class="container">        <div class="progress progress-striped active">            <div class="bar" style="width: ' + rowObj.progress + '%;"></div>        </div>	 <span class="metric">' + rowObj.metric + ' </span> </div>' + 
									"</td>" + 
								   // '<td><div class="progressbar"></div></td>' +
									"<td class='progbaruser'><a href='#' class='uid'>" + (rowObj.uid!=0 ? rowObj.uid : "anon") + "</a></td>" +
									"<td class='timecol'>" + dateToString(time.getMonth(), time.getDate()) + ", " + timeToString(time.getHours(), time.getMinutes()) +
									"</td></tr>";
								
								//if(rowObj.parent+0!=0)
								//	tablerow='<tr><td></td><td colspan="4"><table class="subrow">'+tablerow+'</table></td></tr>'
					
					//			tableRows[rowObj.pid]=tablerow
					//			tableRows[rowObj.parent].append 
